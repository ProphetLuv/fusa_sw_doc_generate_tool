...
5 Agent 架构：SRS / SAD / FMEA / SDD / TC 各一个独立 Agent
"""
...
<style>
    ...
    .agent-card {
        ...
        text-align: center; transition: transform 0.2s; min-height: 200px;
    }
    .agent-card:hover { transform: translateY(-4px); box-shadow: 0 8px 25px rgba(0,0,0,0.15); }
    .agent-card h3 { margin: 8px 0 4px 0; font-size: 1.3rem; }
...
_AGENT_META = {
    ...
    "TC":   {"icon": "🧪", "name": "TC Agent",  "full": "测试用例",
             "desc": "生成覆盖所有分支和路径的测试用例，含覆盖矩阵和通过准则",
...
def _render_dashboard(config: dict):
    """渲染 Agent 仪表盘：5 张卡片 + 批量操作 + 已完成文档汇总。"""
    ...
    for col, agent_type in zip(cols, ["SRS", "SAD", "FMEA", "SDD", "TC"]):
        ...
        with col:
            ...
            <div style="background: {meta['color']}; border-radius: 12px; padding: 20px;
                        color: white; text-align: center; min-height: 180px;">
                ...
                <p style="font-size: 0.75rem; opacity: 0.75;">{meta['desc'][:40]}...</p>
    ...
    if docs:
        st.markdown("---")
        st.markdown(f"### 📄 已生成文档汇总（{len(docs)} / 5）")
...
def _batch_generate_all(config: dict):
    """按 SRS→SAD→FMEA→SDD→TC 顺序批量生成全部文档。"""
    order = ["SRS", "SAD", "FMEA", "SDD", "TC"]
    code = st.session_state.shared_code
