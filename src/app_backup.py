...
if "cancel_generation" not in st.session_state:
    st.session_state.cancel_generation = False  # 取消标志
if "last_failed_agent" not in st.session_state:
    st.session_state.last_failed_agent = None  # 上次失败的 Agent
...
def _render_manual_mode():
    ...
    masked_key = _mask_api_key(api_key) if api_key else ""
    export_cfg = {"provider": provider, "api_key": masked_key, "api_base": api_base or "",
...
def _render_code_input() -> str:
    ...
    with tab_upload:
        ...
        if uploaded_files:
            ...
            parse_progress.empty()  # 解析完成后移除进度条
