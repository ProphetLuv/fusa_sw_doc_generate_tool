...
class PromptManager:
    ...
    def get_prompt(
        ...
        Args:
            doc_type:         文档类型，可选 SRS / SAD / FMEA / SDD / TC
            code:             待分析的 C/C++ 源代码
...
请确保设计文档与代码实现严格对应，每个设计元素都能追溯到具体代码。"""
    ...
    def _build_tc_prompt(self, code: str, module_name: str, asil_level: str) -> str:
        """构建测试用例文档生成 Prompt"""
        return f"""你是一名功能安全测试工程师，请根据以下 C/C++ 嵌入式代码，设计完整的**测试用例文档（TC）**，符合 ISO 26262 Part 6 测试要求和 ASPICE SWE.5 集成测试规范。
...
请按照以下结构输出 Markdown 格式的测试用例文档：

# {module_name} 测试用例文档（TC）

## 1. 测试策略
简述测试方法、覆盖目标（MC/DC / 分支覆盖 / 语句覆盖）、测试环境假设。

## 2. 测试用例表

请使用以下五类测试设计技术，为代码中的每个函数设计测试用例：
...
### 2.5 安全机制测试
针对故障检测、安全状态转换、看门狗、冗余校验等安全机制设计测试。
...
要求：
- **测试用例ID**：格式 TC-MOD-XXX（XXX为三位序号）
- **优先级**：高(安全相关) / 中(功能正确性) / 低(边界/异常)
- **追溯需求**：关联到 SRS 需求ID（可虚拟）

## 3. 测试代码框架

请为关键测试用例生成 Unity（C语言）和 Google Test（C++）两种框架的测试代码：
...
## 5. 测试通过准则
各 ASIL 等级对应的覆盖率要求、通过率要求。

请确保测试用例全面覆盖代码的所有分支和路径，特别关注安全相关的测试场景。"""
    ...
    DOC_CHUNKS = {
        ...
        "TC": [
            {"id": 1, "title": "第1~3章", "sections": "1. 测试策略、2. 测试环境、3. 功能测试用例"},
