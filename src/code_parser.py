...
from llm_engine import estimate_tokens as _estimate_tokens
...
class CodeParser:
    ...
    def _init_tree_sitter(self):
        ...
        try:
            ...
            try:
                from tree_sitter import Language, Parser
                import tree_sitter_c as _ts_c_mod
                import tree_sitter_cpp as _ts_cpp_mod
                self._ts_c_lang = Language.create(_ts_c_mod.language())
                self._ts_cpp_lang = Language.create(_ts_cpp_mod.language())
                self._ts_parser = Parser()
                self._ts_available = True
            except Exception:
                self._ts_available = False
    ...
    def analyze(self, code: str) -> Dict[str, Any]:
        ...
        # 统一使用 llm_engine 的 estimate_tokens 算法，避免两套逻辑不一致
        estimated_tokens = max(1, _estimate_tokens(code))
    ...
    def _analyze_with_tree_sitter(self, code: str) -> Tuple[List[str], Dict[str, List[str]], Dict[str, int]]:
        ...
        is_cpp = any(ext in code for ext in ['::', 'class ', 'template ', 'namespace '])
    ...
    def _extract_call_graph_regex(self, code: str, func_names: List[str]) -> Dict[str, List[str]]:
        ...
        # 预编译所有函数名的调用模式（避免内层循环重复 compile）
        call_patterns = {
        ...
        for match in func_matches:
            ...
            called = []
            for other_name, pattern in call_patterns.items():
                if other_name == fname:
                    continue
